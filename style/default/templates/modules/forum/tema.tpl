                    ...
                    {if $arrayrowfile && $rows.count_file > 0}
                        ...
                        {foreach from=$arrayrowfile item=file}
                            {if $rows.id == $file.id_post}
                                <p class="menu"><a href="{$home}/forum/{$forum.id}/{$row.id}/{$tema.id}/load/{$file.id}" title="Скачать файл"><i class="fa fa-file"></i> {$file.file|escape|esc}</a> ({$file.size|esc}{if $file.loadcounts > 0}, скачиваний: {$file.loadcounts|number}{/if})</p>   
                            {/if}
                        {/foreach}
                    {/if}
                    {if $user.level > 30}
                        <p><a href="{$home}/forum/setup/post/{$rows.id}">редактировать</a>{if $count > 1} / <a href="{$home}/forum/setup/post/del/{$rows.id}">удалить</a>{/if}</p>
                    {/if}
                </div>
            {/foreach}
        {else}
